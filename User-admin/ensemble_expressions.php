  ...
  <div class="wrapper">
    ...
    <main class="flex-1 p-6">
      <div class="bg-white shadow-sm rounded-lg p-4 mb-4 flex justify-between items-center">
        <button class="validate-btn">Tableau De Bord Stock</button>
      ...
      <div class="p-4 mt-8">
        <div class="header-container mb-4 flex items-center justify-between">
          <h2 class="text-2xl font-semibold">Historique Des Expressions</h2>
        </div>

        <div id="besoin-view" class="view-container">
          <div class="grid grid-cols-1 md:grid-cols-1 gap-6">
            <div class="bg-white p-4 rounded-lg shadow">
              <h3 class="text-lg font-semibold mb-2"></h3>
              <hr>
              <br>
              <div id="today-cards-besoin" class="grid grid-cols-1 md:grid-cols-6 gap-4"></div>
            </div>
          </div>
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchAndDisplayCardsBesoin() {
        fetch(`get_ens_expressions_vaide.php`) // URL pour les besoins validés
            .then(response => response.json())
            .then(data => {
                const container = document.getElementById('today-cards-besoin');
                container.innerHTML = '';
                data.forEach(expr => {
            ...
            .catch(error => console.error(`Error fetching expressions besoin:`, error));
    }

    // Fonction de recherche
    function filterCards(searchTerm) {
        const allCardElements = document.querySelectorAll(`#today-cards-besoin .card`);
        allCardElements.forEach(card => {
            const cardText = card.innerText.toLowerCase();
            card.style.display = cardText.includes(searchTerm.toLowerCase()) ? '' : 'none';
        });
    }
