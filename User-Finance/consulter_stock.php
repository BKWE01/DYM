...
<body>
    ...
    <script>
        $(document).ready(function() {
            ...
            // =============== START MOUVEMENT STOCK ENTREES/SORTIES ================ 

            // Fonction améliorée pour charger les entrées/sorties avec statistiques financières
            function loadEntriesSortiesContent() {
                // Construire le contenu HTML amélioré des entrées/sorties avec statistiques financières
                let entriesSortiesHTML = `
                        <div class="mb-6 flex flex-wrap justify-between items-center">
                            <div class="flex items-center">
                                <h2 class="text-2xl font-bold text-gray-800">Mouvements de Stock</h2>
                                <span class="ml-2 badge badge-blue">Entrées/Sorties</span>
                            </div>

                            <div class="flex flex-wrap space-x-2">
                                <button id="btn-refresh-movements" class="bg-blue-500 text-white px-3 py-2 rounded-md flex items-center hover:bg-blue-600 transition-colors">
                                    <span class="material-icons mr-1" style="font-size: 18px;">refresh</span>
                                    Actualiser
                                </button>
                                <button id="btn-export-movements" class="bg-green-500 text-white px-3 py-2 rounded-md flex items-center hover:bg-green-600 transition-colors">
                                    <span class="material-icons mr-1" style="font-size: 18px;">file_download</span>
                                    Exporter
                                </button>
                            </div>
                        ...
                        <!-- Statistiques financières des mouvements -->
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                            <!-- Valeur des Entrées -->
                            <div class="dashboard-card bg-gradient-to-r from-green-50 to-green-100 border-l-4 border-green-500">
                                <div class="p-4 flex justify-between items-center">
                                    <div>
                                        <p class="text-sm font-medium text-green-700">Valeur Entrées</p>
                                        <h3 id="financial-stats-entries-value" class="text-2xl font-bold text-green-800">-</h3>
                                        <p class="text-xs text-green-600 mt-1">
                                            <span id="financial-stats-entries-count">-</span> mouvements
                                        </p>
                                    </div>
                                    <div class="bg-green-200 p-3 rounded-full">
                                        <span class="material-icons text-green-700">trending_up</span>
                                    </div>
                                </div>
                            </div>

                            <!-- Valeur des Sorties -->
                            <div class="dashboard-card bg-gradient-to-r from-red-50 to-red-100 border-l-4 border-red-500">
                                <div class="p-4 flex justify-between items-center">
                                    <div>
                                        <p class="text-sm font-medium text-red-700">Valeur Sorties</p>
                                        <h3 id="financial-stats-outputs-value" class="text-2xl font-bold text-red-800">-</h3>
                                        <p class="text-xs text-red-600 mt-1">
                                            <span id="financial-stats-outputs-count">-</span> mouvements
                                        </p>
                                    </div>
                                    <div class="bg-red-200 p-3 rounded-full">
                                        <span class="material-icons text-red-700">trending_down</span>
                                    </div>
                                </div>
                            </div>

                            <!-- Bilan Net -->
                            <div class="dashboard-card bg-gradient-to-r from-blue-50 to-blue-100 border-l-4 border-blue-500">
                                <div class="p-4 flex justify-between items-center">
                                    <div>
                                        <p class="text-sm font-medium text-blue-700">Bilan Net</p>
                                        <h3 id="financial-stats-net-value" class="text-2xl font-bold text-blue-800">-</h3>
                                        <p class="text-xs text-blue-600 mt-1">
                                            Différence Entrées - Sorties
                                        </p>
                                    </div>
                                    <div class="bg-blue-200 p-3 rounded-full">
                                        <span class="material-icons text-blue-700" id="net-value-icon">account_balance</span>
                                    </div>
                                </div>
                            </div>

                            <!-- Prix Moyen -->
                            <div class="dashboard-card bg-gradient-to-r from-purple-50 to-purple-100 border-l-4 border-purple-500">
                                <div class="p-4 flex justify-between items-center">
                                    <div>
                                        <p class="text-sm font-medium text-purple-700">Prix Moyen</p>
                                        <h3 id="financial-stats-avg-price" class="text-lg font-bold text-purple-800">-</h3>
                                        <p class="text-xs text-purple-600 mt-1">
                                            Par unité de mouvement
                                        </p>
                                    </div>
                                    <div class="bg-purple-200 p-3 rounded-full">
                                        <span class="material-icons text-purple-700">analytics</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div id="period-info" class="mb-4 p-3 bg-blue-50 border-l-4 border-blue-400 rounded-r-lg" style="display: none;">
                            <div class="flex items-center">
                                <span class="material-icons text-blue-500 mr-2">date_range</span>
                                <div>
                                    <p class="text-blue-800 font-medium">Période analysée</p>
                                    <p id="period-details" class="text-blue-600 text-sm">-</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card-container">
                            ...
                            <div class="filter-section">
                                ...
                                <div class="filter-group">
                                    <div class="filter-item">
                                        <label class="filter-label">Recherche produit</label>
                                        <input type="text" id="filter-product" class="filter-input" placeholder="Nom du produit">
                                    </div>
                                    ...
                                    <div class="filter-item">
                                        ...
                                        <select id="filter-movement-type" class="filter-input">
                                            ...
                                            <option value="entry">Entrées (classiques + dispatching)</option>
                                            <option value="dispatch">Dispatching uniquement</option>
                                    ...
                                    <div class="filter-item">
                                        ...
                                        <select id="filter-project-movements" class="filter-input">
                ...
                // Charger les statistiques financières initiales
                loadFinancialStats();
            }

            // Nouvelle fonction pour charger les statistiques financières
            function loadFinancialStats() {
                const filters = {
                    movement_type: $('#filter-movement-type').val(),
                    project_code: $('#filter-project-movements').val(),
                    date_from: $('#filter-date-from').val(),
                    date_to: $('#filter-date-to').val(),
                    product_filter: $('#filter-product').val(),
                    search: window.movementsTable ? window.movementsTable.search() : ''
                };

                // Construire l'URL avec les filtres
                let url = 'api/stock/entrees_sorties/get_movements_financial_stats.php';
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success) {
                            updateFinancialStatsDisplay(data.stats);
                            updatePeriodInfo(data.stats.periode, data.filters_applied);
                        } else {
                            console.error('Erreur lors du chargement des statistiques financières:', data.message);
                            showDefaultFinancialStats();
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Erreur AJAX lors du chargement des statistiques financières:', error);
                        showDefaultFinancialStats();
                    }
                });
            }

            // Fonction pour mettre à jour l'affichage des statistiques financières
            function updateFinancialStatsDisplay(stats) {
                // Valeurs des entrées
                $('#financial-stats-entries-value').text(stats.valeur_entries_formatted || '0 FCFA');
                $('#financial-stats-entries-count').text(stats.total_entries || 0);

                // Valeurs des sorties
                $('#financial-stats-outputs-value').text(stats.valeur_outputs_formatted || '0 FCFA');
                $('#financial-stats-outputs-count').text(stats.total_outputs || 0);

                // Bilan net
                const netValue = stats.valeur_nette || 0;
                $('#financial-stats-net-value').text(stats.valeur_nette_formatted || '0 FCFA');

                // Changer la couleur et l'icône selon le bilan
                const netContainer = $('#financial-stats-net-value').closest('.dashboard-card');
                const netIcon = $('#net-value-icon');

                if (netValue > 0) {
                    netContainer.removeClass('from-blue-50 to-blue-100 border-blue-500')
                        .addClass('from-green-50 to-green-100 border-green-500');
                    $('#financial-stats-net-value').removeClass('text-blue-800').addClass('text-green-800');
                    netIcon.removeClass('text-blue-700').addClass('text-green-700').text('trending_up');
                } else if (netValue < 0) {
                    netContainer.removeClass('from-blue-50 to-blue-100 border-blue-500')
                        .addClass('from-red-50 to-red-100 border-red-500');
                    $('#financial-stats-net-value').removeClass('text-blue-800').addClass('text-red-800');
                    netIcon.removeClass('text-blue-700').addClass('text-red-700').text('trending_down');
                } else {
                    netContainer.removeClass('from-green-50 to-green-100 border-green-500 from-red-50 to-red-100 border-red-500')
                        .addClass('from-blue-50 to-blue-100 border-blue-500');
                    $('#financial-stats-net-value').removeClass('text-green-800 text-red-800').addClass('text-blue-800');
                    netIcon.removeClass('text-green-700 text-red-700').addClass('text-blue-700').text('account_balance');
                }

                // Prix moyen
                const avgPrice = (stats.prix_moyen_entrees + stats.prix_moyen_sorties) / 2;
                $('#financial-stats-avg-price').text(
                    avgPrice > 0 ?
            ...
            // Fonction pour afficher les statistiques par défaut
            function showDefaultFinancialStats() {
                $('#financial-stats-entries-value').text('0 FCFA');
                $('#financial-stats-entries-count').text('0');
                $('#financial-stats-outputs-value').text('0 FCFA');
                $('#financial-stats-outputs-count').text('0');
                $('#financial-stats-net-value').text('0 FCFA');
                $('#financial-stats-avg-price').text('N/A');
            }

            // Fonction pour mettre à jour les informations de période
            function updatePeriodInfo(periode, filters) {
                if (periode && periode.filtres_appliques) {
                    let periodText = '';

                    if (periode.date_debut && periode.date_fin) {
                        const dateDebut = new Date(periode.date_debut).toLocaleDateString('fr-FR');
                        const dateFin = new Date(periode.date_fin).toLocaleDateString('fr-FR');
                        periodText = `Du ${dateDebut} au ${dateFin}`;
                    } else if (filters.date_from || filters.date_to) {
                        if (filters.date_from && filters.date_to) {
                            const dateFrom = new Date(filters.date_from).toLocaleDateString('fr-FR');
                            const dateTo = new Date(filters.date_to).toLocaleDateString('fr-FR');
                            periodText = `Filtrée: du ${dateFrom} au ${dateTo}`;
                        } else if (filters.date_from) {
                            const dateFrom = new Date(filters.date_from).toLocaleDateString('fr-FR');
                            periodText = `Filtrée: à partir du ${dateFrom}`;
                        } else if (filters.date_to) {
                            const dateTo = new Date(filters.date_to).toLocaleDateString('fr-FR');
                            periodText = `Filtrée: jusqu'au ${dateTo}`;
                        }
                    } else {
                        periodText = 'Toutes les données disponibles';
                    }

                    $('#period-details').text(periodText);
                    $('#period-info').show();
                } else {
                    $('#period-info').hide();
                }
            ...
            // Initialisation de DataTables pour les mouvements - VERSION AMÉLIORÉE
            function initMovementsDataTable() {
                if ($.fn.DataTable.isDataTable('#movements-table')) {
                    $('#movements-table').DataTable().destroy();
                }

                window.movementsTable = $('#movements-table').DataTable({
                    processing: true,
                    serverSide: true,
                    responsive: true,
                    ajax: {
                        url: 'api/stock/get_stock_movements_enhanced.php',
                        type: 'GET',
                        data: function(d) {
                            // Ajouter les filtres personnalisés
                            d.movement_type = $('#filter-movement-type').val();
                            d.project_code = $('#filter-project-movements').val();
                            d.date_from = $('#filter-date-from').val();
                            d.date_to = $('#filter-date-to').val();
                            d.product_filter = $('#filter-product').val();
                        },
                        dataSrc: function(json) {
                            // Mettre à jour les statistiques financières après chaque chargement
                            setTimeout(loadFinancialStats, 500);
                            return json.data;
                        },
                        error: function(xhr, status, error) {
                            console.error("Erreur AJAX:", status, error);
                    ...
                    columns: [{
                        ...
                        {
                            // Nouvelle colonne pour la valeur unitaire
                            data: null,
                            width: "120px",
                            className: "text-right",
                            render: function(data, type, row) {
                                // Calculer la valeur basée sur les données du mouvement
                                // Cette valeur sera calculée côté serveur dans une version future
                                return '<span class="text-green-600 font-medium">À calculer</span>';
                            }
                        },
                        {
                            data: 9,
                            width: "120px"
                        }, // Facture (anciennement colonne 9)
                        {
                            data: 10,
                            width: "80px",
                            orderable: false
                        } // Actions (anciennement colonne 10)
                    ],
                    dom: 'Bfrtip',
                    buttons: [{
                            extend: 'excel',
                            text: '<span class="material-icons" style="font-size: 16px; vertical-align: middle;">file_download</span> Excel',
                            className: 'btn-filter',
                            title: 'Mouvements_Stock_Financier',
                        ...
                        },
                        {
                            ...
                            text: '<span class="material-icons" style="font-size: 16px; vertical-align: middle;">print</span> Imprimer',
                            className: 'btn-filter',
                            title: 'Mouvements de Stock - Analyse Financière',
                            action: function(e, dt, button, config) {
                                exportAllData('print');
                            }
                        }
                    ...
                    drawCallback: function(settings) {
                        ...
                        // Améliorer l'accessibilité des boutons d'action
                    ...
                    initComplete: function() {
                        console.log('DataTable des mouvements avec statistiques financières initialisée avec succès');

                        // Ajouter un indicateur de chargement personnalisé
            ...
            // Mise à jour des statistiques des mouvements
            function updateMovementStats(stats) {
                $('#stats-entries').text(stats.total_entries || 0);
                $('#stats-outputs').text(stats.total_outputs || 0);
                $('#stats-returns').text(stats.total_returns || 0);
                $('#stats-transfers').text(stats.total_transfers || 0);

                // Ajouter des informations sur les quantités
                const netQuantity = (parseFloat(stats.quantity_entries) || 0) - (parseFloat(stats.quantity_outputs) || 0);
                const netColor = netQuantity >= 0 ? 'text-green-600' : 'text-red-600';

                $('#stats-entries').parent().find('p').text(`Total Entrées (${Number(stats.quantity_entries || 0).toLocaleString()} unités)`);
                $('#stats-outputs').parent().find('p').text(`Total Sorties (${Number(stats.quantity_outputs || 0).toLocaleString()} unités)`);
            }

            // Charger les projets pour le filtre des mouvements
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success && response.projects) {
                            const projectSelect = $('#filter-project-movements');
                            projectSelect.empty();
            ...
            // Configuration des événements pour les filtres de mouvements - VERSION AMÉLIORÉE
            function setupMovementFilters() {
                // Boutons d'action
                $('#btn-refresh-movements').on('click', function() {
                    if (window.movementsTable) {
                        window.movementsTable.ajax.reload();
                        loadFinancialStats();

                        Toastify({
                            text: "Données financières actualisées",
                            duration: 2000,
                            gravity: "top",
                            position: "right",
                ...
                $('#btn-export-movements').on('click', function() {
                    if (window.movementsTable) {
                        window.movementsTable.button('.buttons-excel').trigger();
                    }
                });

                // Filtres avec mise à jour des statistiques
                ...
                // Application automatique des filtres avec debounce et mise à jour des stats
                $('#filter-product').on('input', debounce(function() {
                    if (window.movementsTable) {
                        window.movementsTable.ajax.reload();
                        loadFinancialStats();
                    }
                ...
                $('#filter-movement-type, #filter-project-movements, #filter-date-from, #filter-date-to').on('change', function() {
                    if (window.movementsTable) {
                        window.movementsTable.ajax.reload();
                        loadFinancialStats();
                    }
            ...
            function number_format(number, decimals, dec_point, thousands_sep) {
                ...
                const sign = number < 0 ? '-' : '';
                const absNumber = Math.abs(number).toFixed(decimals);
                const parts = absNumber.split('.');
                const integerPart = parts[0];
                const decimalPart = parts[1] ? dec_point + parts[1] : '';
            ...
            // Fonction pour exporter toutes les données - VERSION CORRIGÉE
            function exportAllData(format) {
                // Afficher un indicateur de chargement
                Toastify({
                    text: "Préparation de l'export en cours...",
                    duration: 2000,
                    gravity: "top",
                    position: "right",
                ...
                // Construire l'URL avec les filtres - UTILISER LE NOUVEAU FICHIER
                let exportUrl = 'api/stock/pdf/export_movements.php?format=' + format;
                ...
                // Pour Excel et PDF, télécharger le fichier
                if (format === 'excel' || format === 'pdf') {
                    ...
                    setTimeout(() => {
                        Toastify({
                            text: "Export " + (format === 'excel' ? 'Excel' : 'PDF') + " généré avec succès!",
                            duration: 3000,
                            gravity: "top",
                            position: "right",
                            backgroundColor: "#10b981",
                    ...
                    // Pour l'impression, ouvrir dans une nouvelle fenêtre
            ...
            window.viewMovementDetails = function(movementId) {
                ...
                $('#productModalTitle').text(`Détails du Mouvement #${movementId}`);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.success && data.movement) {
                            ...
                            showMovementError('Impossible de charger les détails du mouvement.');
                        }
                    },
                    error: function(xhr, status, error) {
                        console.error('Erreur lors du chargement des détails:', error);
                        showMovementError('Erreur lors du chargement des détails du mouvement.');
                    }
                });
            };
            ...
            function displayMovementDetails(movement) {
                ...
                switch (movement.movement_type) {
                    ...
                    case 'transfer':
                        ...
                        break;
                    default:
                        typeDisplay = movement.movement_type;
                        typeColor = 'text-gray-600';
                }

                const formattedDate = new Date(movement.date).toLocaleDateString('fr-FR', {
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
                    hour: '2-digit',
                ...
                let html = `
                    <div class="movement-details-section grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <div class="text-sm font-medium text-gray-500 mb-1">Type de mouvement</div>
                            <div class="font-semibold text-lg ${typeColor} mb-3">${typeDisplay}</div>

                            <div class="text-sm font-medium text-gray-500 mb-1">Produit</div>
                            <div class="font-semibold text-lg mb-3">${movement.product_name}</div>

                            <div class="text-sm font-medium text-gray-500 mb-1">Catégorie</div>
                            <div class="mb-3"><span class="category-tag">${movement.category_name || 'Non catégorisé'}</span></div>
                        ...
                        <div>
                            <div class="text-sm font-medium text-gray-500 mb-1">Quantité</div>
                            <div class="font-bold text-2xl ${typeColor} mb-3">${movement.quantity} ${movement.unit || ''}</div>

                            ${isEntry || isReturn ? `
                                <div class="text-sm font-medium text-gray-500 mb-1">Provenance</div>
                                <div class="font-medium mb-3">${movement.provenance || '-'}</div>
                            ` : ''}

                            ${isOutput || isTransfer ? `
                                <div class="text-sm font-medium text-gray-500 mb-1">Destination</div>
                                <div class="font-medium mb-3">${movement.destination || '-'}</div>

                                <div class="text-sm font-medium text-gray-500 mb-1">Demandeur</div>
                                <div class="font-medium mb-3">${movement.demandeur || '-'}</div>
                            ` : ''}

                            <div class="text-sm font-medium text-gray-500 mb-1">Projet</div>
                            <div class="mb-3">${movement.nom_projet ? '<span class="badge badge-blue">' + movement.nom_projet + '</span>' : '-'}</div>
                        </div>
                    </div>

                    ${movement.notes ? `
                        <div class="movement-details-section mt-6">
                    ...
                    ${movement.invoice_id ? `
                        <div class="movement-details-section mt-6">
                            <h3 class="font-semibold text-lg mb-3">Facture associée</h3>
                            <div class="bg-blue-50 p-4 rounded-lg">
                                <button class="btn btn-primary" onclick="previewInvoice(${movement.invoice_id})">
                                    <span class="material-icons mr-2">description</span>
                                    Voir la facture #${movement.invoice_id}
                                </button>
                            </div>
                        </div>
                    ` : ''}
                ...
                // Masquer le bouton d'historique et configurer la fermeture
                $('#viewProductHistory').hide();
            }

            // Fonction pour afficher une erreur de mouvement
            ...
            // =============== END MOUVEMENT STOCK ENTREES/SORTIES ================ 
            ...
            function updateTopProductsList(topProducts) {
                ...
                topProducts.forEach((product, index) => {
                    const rank = index + 1;
                    html += `
                    <div class="product-status-item flex flex-wrap items-center justify-between">
                        <div class="flex items-center">
