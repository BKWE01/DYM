<?php
/**
 * api_get_archived_orders.php
 * API pour récupérer les bons de commande en attente de signature Finance
 * 
 * @author DYM MANUFACTURE
 * @version 2.0
 */

// Désactiver l'affichage des erreurs pour les API (pour éviter les erreurs HTML)
...
try {
    ...
    // Récupérer les bons en attente
    $response = $financeController->getPendingOrders();
    ...
    $response->addMeta('total_records', count($response->getData() ?? []));
    ...
    error_log("Erreur API get_archived_orders: " . $e->getMessage());
    
    ApiResponse::error(
        'Erreur technique lors de la récupération des bons de commande',
