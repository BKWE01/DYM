...
try {
    ...
    // Joindre avec la table products pour obtenir des informations supplémentaires sur les produits
    $stmt = $pdo->prepare("
    ...
    $userIds = array_filter([$userEmetId, $userStockId, $userAchatId]); // Filtrer les valeurs NULL
...
class PDF extends FPDF {
    ...
    function Header() {
        // Logo
        $this->Image('../../../public/logo.png', 25, 12, 30); // Ajustez la position et la taille du logo
    ...
    function besoinsTable($expressions) {
        // En-tête du tableau
        $this->SetFont('Times', 'B', 12);
        $this->Cell(130, 10, utf8_decode('Désignation'), 1, 0, 'C');
        $this->Cell(20, 10, utf8_decode('Qté Dem.'), 1, 0, 'C');
        $this->Cell(20, 10, utf8_decode('Qté Stock'), 1, 0, 'C');
        $this->Cell(20, 10, utf8_decode('Qté Achat'), 1, 1, 'C');

        // Contenu du tableau
        $this->SetFont('Times', '', 12);
        foreach ($expressions as $row) {
            // Définir l'unité (pièce par défaut si non spécifiée)
            ...
            // Limiter le texte à la largeur de la cellule
            $designation = $row['designation_article'];
            if (strlen($designation) > 70) { // Ajustez cette valeur selon vos besoins
                $designation = substr($designation, 0, 67) . '...';
            }
            
            $this->Cell(130, 10, utf8_decode($designation), 1, 0, 'L');
            $this->Cell(20, 10, utf8_decode($qt_demande), 1, 0, 'C');
            $this->Cell(20, 10, utf8_decode($qt_stock), 1, 0, 'C');
            $this->Cell(20, 10, utf8_decode($qt_acheter), 1, 1, 'C');
        }
    }
    ...
    function Footer() {
        ...
        $this->Cell($colWidth, 10, utf8_decode('VISA Demandeur'), 1, 0, 'C');
