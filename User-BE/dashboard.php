...
try {
  ...
  $receivedQuery = "SELECT COUNT(DISTINCT am.id) as count, ed.valide_achat
  FROM achats_materiaux am 
  INNER JOIN expression_dym ed ON (am.expression_id = ed.idExpression AND am.designation = ed.designation)
  WHERE am.status = 'reçu' 
  AND am.parent_id IS NULL
  AND am.date_reception >= DATE_SUB(NOW(), INTERVAL 30 DAY)
