...
<body>
                ...
                <div id="pending-expressions-container">
                    ...
                    <div id="today-content" class="tab-content active">
                        ...
                        <div id="today-no-data" class="hidden text-center py-8">
                            ...
                            <p class="text-gray-500 font-medium">Aucune expression non validée aujourd'hui</p>
                        </div>
                    </div>
                    ...
                    <div id="week-content" class="tab-content">
                        ...
                        <div id="week-no-data" class="hidden text-center py-8">
                            <span class="material-icons text-gray-400 text-5xl mb-2">inbox</span>
                            <p class="text-gray-500 font-medium">Aucune expression non validée cette semaine</p>
                        </div>
                    </div>
                    ...
                    <div id="month-content" class="tab-content">
                        ...
                        <div id="month-no-data" class="hidden text-center py-8">
                            <span class="material-icons text-gray-400 text-5xl mb-2">inbox</span>
                            <p class="text-gray-500 font-medium">Aucune expression non validée ce mois-ci</p>
                        </div>
                    </div>
                </div>

                <!-- Validated Expressions View (initially hidden) -->
                <div id="validated-expressions-container" class="hidden">
                    <!-- Tab Content for Validated -->
    ...
    <script>
        // Solution complète pour dashboard.php
        $(document).ready(function () {
            ...
            $('.card-tab').on('click', function () {
                ...
                const currentView = $('#pending-expressions-container').is(':visible') ? '' : '-validated';
                ...
                $(`#${tabId}-content${currentView}`).addClass('active');
            });
            ...
            $('#pending-view-btn').on('click', function () {
                ...
                // Make sure the active tab content is displayed
                const activeTabId = $('.card-tab.active').data('tab');
                $('.tab-content').removeClass('active');
            ...
            $('#validated-view-btn').on('click', function () {
                ...
                const activeTabId = $('.card-tab.active').data('tab');
                $('.tab-content').removeClass('active');
                $(`#${activeTabId}-content-validated`).addClass('active');
            });

            // Configuration spécifique pour chaque tableau
            ...
            // Initialize DataTables for pending expressions
            function initializePendingTable(period, data) {
                $(`#${period}-loading`).hide();
                ...
                try {
                    ...
                    // Utiliser les configurations spécifiques selon la période
                    const config = period === 'today' ? todayTableConfig : weekMonthTableConfig;
                    ...
                    $(`#${period}-table tbody`).on('click', 'tr', function () {
                        const rowData = table.row(this).data();
                        if (rowData && rowData.idExpression) {
                            window.location.href = `expression_details.php?id=${rowData.idExpression}`;
                        }
                    });
                } catch (error) {
            ...
            function initializeValidatedTable(period, data) {
                ...
                try {
                    ...
                    $(`#${period}-table-validated tbody`).on('click', 'tr', function () {
                        const rowData = table.row(this).data();
                        if (rowData && rowData.idExpression) {
                            window.open(`generate_pdf.php?id=${rowData.idExpression}`, '_blank');
                        }
                    });
                } catch (error) {
            ...
            // Load data for pending expressions avec gestion d'erreurs améliorée
            function loadPendingData(period) {
                $.ajax({
                    url: `get_expressions.php?period=${period}`,
                    type: 'GET',
                    dataType: 'json',
                    success: function (data) {
                        initializePendingTable(period, data);
                    },
                    error: function (xhr, status, error) {
                        ...
                        // En cas d'erreur, tenter d'initialiser avec un tableau vide
                        initializePendingTable(period, []);
                    }
                });
            }

            // Load data for validated expressions avec gestion d'erreurs améliorée
            function loadValidatedData(period) {
                $.ajax({
                    ...
                    error: function (xhr, status, error) {
                        ...
                        // En cas d'erreur, tenter d'initialiser avec un tableau vide
                        initializeValidatedTable(period, []);
                    }
                });
            }

            // Charger les données séquentiellement pour éviter les conflits
            setTimeout(() => loadPendingData('today'), 0);
            setTimeout(() => loadPendingData('week'), 300);
            setTimeout(() => loadPendingData('month'), 600);

            setTimeout(() => loadValidatedData('today'), 900);
            setTimeout(() => loadValidatedData('week'), 1200);
            setTimeout(() => loadValidatedData('month'), 1500);
        });
    </script>
</body>
